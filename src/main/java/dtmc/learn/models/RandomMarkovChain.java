...
import dtmc.learn.utils.IntegerUtil;
...
public class RandomMarkovChain {
	...
	public static void main(String[] args) throws FileNotFoundException, IOException{
		...
		RandomMarkovChain rmc = new RandomMarkovChain(node_number, density, "rmc-8-1");
		rmc.generateRMC();
		double[] rmc_id = new double[node_number];
		for(int i=0; i<node_number/2; i++){
			rmc_id[i] = (double)2/node_number;
		}
		List<Integer> rmc_ts = new ArrayList<Integer>();
		for(int i=node_number/2; i<node_number; i++){
			rmc_ts.add(i);
		}

		RealVector rmc_init_dist = MatrixUtils.createRealVector(rmc_id);
		RealMatrix rmc_transition_matrix = rmc.getTransitionMatrix();
		Model rmc_model = new Model("rmc", rmc_transition_matrix, rmc_init_dist, rmc_ts);
		rmc_model.saveModel();
