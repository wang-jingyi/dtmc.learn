...
public class PassiveLearnExperiment implements Experiment{
	...
	public ExperimentResult runExperiment(Config config) throws FileNotFoundException, ClassNotFoundException, IOException {
		...
		Reachability learned_reach = new Reachability(learned_mc.getTransitionMatrix(), init_states, init_states_probs, target_states,
				System.getProperty("user.dir")+"/resources/tmp/learned_reach/"+model.getModel_name(), model.getModel_name()+"_actual", reach_bounded_step);
		List<Double> learned_target_states_reach = learned_reach.computeReachability();
		
		ExperimentResult er = new ExperimentResult(min_visit, mse, target_states, target_states_reach, learned_target_states_reach);
